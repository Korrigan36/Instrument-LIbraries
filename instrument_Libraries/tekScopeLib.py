...
#        print(tempString)
        ...
        if channel.find("CH1") != -1:
            tempString = str(channel) + ":LABel:XPOS 50" 
#            scopeObject.write(tempString)
            tempString = str(channel) + ":LABel:YPOS 100" 
            scopeObject.write(tempString)
        if channel.find("CH2") != -1:
            tempString = str(channel) + ":LABel:XPOS 50" 
            scopeObject.write(tempString)
            tempString = str(channel) + ":LABel:YPOS 90" 
            scopeObject.write(tempString)
        if channel.find("CH3") != -1:
            tempString = str(channel) + ":LABel:XPOS 50" 
            scopeObject.write(tempString)
            tempString = str(channel) + ":LABel:YPOS 50" 
            scopeObject.write(tempString)
        if channel.find("CH4") != -1:
            tempString = str(channel) + ":LABel:XPOS 50" 
            scopeObject.write(tempString)
            tempString = str(channel) + ":LABel:YPOS 50" 
            scopeObject.write(tempString)
        if channel.find("CH5") != -1:
            tempString = str(channel) + ":LABel:XPOS 50" 
            scopeObject.write(tempString)
            tempString = str(channel) + ":LABel:YPOS 70" 
            scopeObject.write(tempString)
        if channel.find("CH6") != -1:
            tempString = str(channel) + ":LABel:XPOS 50" 
            scopeObject.write(tempString)
            tempString = str(channel) + ":LABel:YPOS 50" 
            scopeObject.write(tempString)
        if channel.find("CH7") != -1:
            tempString = str(channel) + ":LABel:XPOS 50" 
            scopeObject.write(tempString)
            tempString = str(channel) + ":LABel:YPOS 50" 
            scopeObject.write(tempString)
        if channel.find("CH8") != -1:
            tempString = str(channel) + ":LABel:XPOS 50" 
            scopeObject.write(tempString)
            tempString = str(channel) + ":LABel:YPOS 50" 
            scopeObject.write(tempString)            
...
def scope_SetAcquisitionSample(scopeObject):
    tempString = "ACQuire:MODE SAMple"
    scopeObject.write(tempString)
    
def scope_SetHorizontalModeAuto(scopeObject):
    """ For use with MSO58 """
    tempString = "HORizontal:MODE AUTO"
    scopeObject.write(tempString)
    
def scope_SetHorizontalModeManual(scopeObject):
    """ For use with MSO58 """
    tempString = "HORizontal:MODE MANual"
    scopeObject.write(tempString)   

def scope_SetSampleRate(scopeObject, sampleRate):
    """ For use with MSO58 """
    tempString = "HORizontal:MODE:SAMPLERate " + str(sampleRate)
    scopeObject.write(tempString)
...
def scope_Reset_Persistance_Only(scopeObject):
    tempString = "DISplay:PERSistence:RESET"
    scopeObject.write(tempString)
...
def scope_SetUp_Trigger(scopeObject, channel, level, edge, position):
    ...
    if returnString.find("4104") != -1:
        ...
        scopeObject.write(tempString)
        tempString = "HORizontal:POSition " + str(position)
        scopeObject.write(tempString)
...
#        print "after read raw"
        ...
        scopeObject.query("*OPC?")  #Make sure the image has been saved before trying to read the file
        #Read file data over
